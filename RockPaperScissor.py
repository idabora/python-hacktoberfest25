...
class RockPaperScissor:
    ...
    def create_user(self):
        ...
        if change == "y":
            rounds = int(input(
                "Enter number of rounds you want to play- ").strip()
                )
    ...
    def evaluate(self, user1_action, user2_action, round_no):
        ...
        print(
            f"{self.user1} move {user1_action},"
            f"{self.user2} move {user2_action}"
            )
    ...
    def final_result(self):
        ...
        if self.user1_score > self.user2_score:
            ...
            print(f"{self.user1} won this game with score "
                  f"{self.user1_score} - {self.user2_score}")

        elif self.user1_score < self.user2_score:

            print(f"{self.user2} won this game with score "
                  f"{self.user2_score} - {self.user1_score}")

        else:

            print(f"This Game ties with score "
                  f"{self.user1_score} - {self.user2_score}")
...
try:
    ...
    for i in range(1, game.rounds + 1):
        user1_action = game.generate_action()
        user2_action = game.generate_action()
